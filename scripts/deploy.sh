#!/usr/bin/env bash
# Deploy ec2 plugin to a single Jenkins instance via Script Console.
...
def version = "${version}"
def url = "https://github.com/nogueiraanderson/ec2-plugin/releases/download/v\${version}/ec2-\${version}.hpi"
def pluginDir = Jenkins.instance.pluginManager.rootDir
def targetFile = new File(pluginDir, "ec2.jpi")
def backupFile = new File(pluginDir, "ec2.jpi.bak")
def pinFile = new File(pluginDir, "ec2.jpi.pinned")
